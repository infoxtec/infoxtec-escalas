...
import { Edit2, ExternalLink, Plus, RefreshCw } from 'lucide-react'
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b bg-muted/50">
                ...
                <SortableTh {...thProps('cliente')}>Cliente</SortableTh>
                <SortableTh {...thProps('endereco')}>Endereço</SortableTh>
                <SortableTh {...thProps('cidade')}>Cidade</SortableTh>
                <SortableTh {...thProps('referencia')}>Referência</SortableTh>
                <SortableTh {...thProps('contato')}>Contato</SortableTh>
                <SortableTh {...thProps('maps')} align="center">Maps</SortableTh>
            ...
            <tbody>
              {sorted.length === 0 ? (
                ...
                <tr key={l.id} className={`border-b last:border-0 hover:bg-muted/30 ${!l.ativo ? 'opacity-50' : ''}`}>
                  <td className="max-w-[160px] truncate px-3 py-2.5 font-medium">{l.nome}</td>
                  <td className="max-w-[120px] truncate px-3 py-2.5 text-xs">{l.cliente ?? '—'}</td>
                  <td className="max-w-[200px] truncate px-3 py-2.5 text-xs text-muted-foreground">{l.endereco ?? '—'}</td>
                  <td className="px-3 py-2.5 text-xs">{l.cidade ?? '—'}</td>
                  <td className="max-w-[120px] truncate px-3 py-2.5 text-xs text-muted-foreground">{l.referencia ?? '—'}</td>
                  <td className="px-3 py-2.5 text-xs">{l.contato_local ? <div><p>{l.contato_local}</p>{l.telefone_contato && <p className="font-mono text-muted-foreground">{l.telefone_contato}</p>}</div> : '—'}</td>
